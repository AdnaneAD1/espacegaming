...
function RejoindreEquipePageContent() {
    ...
    const onSubmit = async (data: JoinTeamFormData) => {
        ...
        // Vérifier si l'équipe est complète (le capitaine est déjà dans players)
        if (teamInfo.players.length >= teamSize) {
                                ...
                                <div className="bg-green-500/10 border border-green-500/30 rounded-lg p-6">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-gray-300">
                                        ...
                                        <div>
                                            ...
                                            <p><span className="font-medium">Joueurs :</span> {teamInfo.players.length}/{teamSize}</p>
                                        </div>
                                    </div>

                                    {teamInfo.players.length >= teamSize && (
                                        <div className="mt-4 p-3 bg-red-500/20 border border-red-500/30 rounded-lg">
                                            <div className="flex items-center text-red-400">
                                                <AlertCircle className="w-5 h-5 mr-2" />
                                                <span className="font-medium">Cette équipe est complète ({teamInfo.players.length}/{teamSize} joueurs)</span>
                                            </div>
                                        </div>
                                    )}
                                </div>
